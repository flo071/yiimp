...
void coinbase_create(YAAMP_COIND *coind, YAAMP_JOB_TEMPLATE *templ, json_value *json_result)
{
	...
	json_int_t dev_amount;
	json_int_t block = json_get_int(json_result, "height");
	if(block < 480000){dev_amount = 10000000;}
	else if(block >= 480000 && block <= 480005){dev_amount = 5000000000000;}
	else {dev_amount = 22000000;}
		
	bool noirnode_masternode_enabled = json_get_bool(json_result, "noirnode_payments_started");
  	if (noirnode_masternode_enabled == true) {
	...
	job_pack_tx(coind, templ->coinb2, dev_amount, script_payee);//dev fee 
